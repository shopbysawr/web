  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    if (!document.getElementById('sidemenu-anim-style')) {
      ...
      animStyle.textContent = [
        ...
        '  background-color: var(--primary-100, #cde5ff) !important;',
        '}',
        // The base theme has "a { transition: 0.2s }" (ALL properties).
        // That makes any transient state (like the theme\'s dark
        // :active outline) fade out visibly instead of disappearing
        // instantly. Only the background is allowed to animate.
        ...
        '}',
        // The chevron is mouse/touch only: never focusable, never any
        // focus ring or press flash, nothing animates except the arrow.
        '.chevron-wrapper,',
        '.chevron-wrapper:hover,',
        '.chevron-wrapper:focus,',
        '.chevron-wrapper:focus-visible,',
        '.chevron-wrapper:focus-within,',
    ...
    function addChevronAndToggle(parentSlug) {
      ...
      // The chevron is not focusable: a press must never move focus
      // anywhere (that's what caused the brief dark ring/edge flash).
      chevronWrapper.addEventListener('mousedown', function(event) {
        event.preventDefault();
      });
      chevronWrapper.addEventListener('touchstart', function() {
